...
const options = Object.keys(commands).filter((key) => !commands[key].noHelp).map((item) => `'${item}'`).join(',')
...
module.exports.install = () => {
  ...
  tryPosixInstall() | tryWindowsInstall()
}
...
module.exports.uninstall = () => {
  posixSnippetManager.tryRemove() | windowsSnippetManager.tryRemove()
}
