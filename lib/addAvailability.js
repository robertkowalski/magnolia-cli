...
var validateAndResolveArgs = function (program) {
  ...
  if (!helper.matchesDefinitionReferenceWithAreaPattern(page) && !helper.matchesDefinitionReferenceWithoutAreaPattern(page)) {
    throw new MgnlCliError(util.format('Expected a value in the form e.g. path-to-page[@area] but was %s', page))
  }
...
function modifyYamlConfiguration (yaml, args) {
  if (!yaml.hasNode('/areas/' + args.targetArea)) {
    ...
    log.info('Created new area %s at the end of %s', args.targetArea, templateScriptPath)
  }
...
function injectComponentAvailability (yamlHelper, args) {
  ...
  if (!yamlHelper.hasNode(availableComponentsPath + '/' + args.component.name)) {
    ...
    log.info('Availability for %s added into %s', args.component.refId, args.templateDefinitionFilePath)
    return true
  } else {
    log.info('Component %s is already available at %s', args.component.refId, args.templateDefinitionFilePath)
  }
  return false
}
...
function injectAutoGeneration (yamlHelper, args) {
  ...
  if (!yamlHelper.hasNode(areaAutoGenerationPath)) {
    ...
    log.info('Autogeneration for %s added into %s', args.component.refId, args.templateDefinitionFilePath)
    return true
  } else {
    log.info('Component %s is already configured for autogeneration at %s', args.component.refId, args.templateDefinitionFilePath)
  }
  return false
}
