...
const downloadMagnolia = require('./downloadMagnolia.js')
const downloadJars = require('./downloadJars.js')
const extractMagnolia = require('./extractMagnolia.js')
const createLightModule = require('./createLightModule.js')
...
const checkMagnoliaVersion = function (version) {
...
const validateAndResolveArgs = function (program, credentials) {
  if (program.enterpriseEdition) {
    if (typeof credentials === 'undefined') {
      throw new MgnlCliError(i18next.t('mgnl-jumpstart--error-invalid-credentials'))
    }
    let url = 'https://nexus.magnolia-cms.com/service/local/artifact/maven/content?r=magnolia.enterprise.releases&g=info.magnolia.eebundle'
   ...
   */
  const lightModulesRoot = process.platform === 'win32' ? path.resolve(program.path).replace(/\\/g, '/') : path.resolve(program.path)
...
function setupMagnolia (program, credentials, cb) {
  const args = validateAndResolveArgs(program, credentials)

  const magnoliaZip = 'magnolia.zip'
