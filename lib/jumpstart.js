...
var checkMagnoliaVersion = function (version) {
  ...
  if (major < 5 || (major === 5 && minor < 4)) {
    throw new MgnlCliError(util.format('Invalid Magnolia version %s, light development is available only for Magnolia 5.4 and greater', version))
  }
}
...
function prepareMagnolia (args, magnoliaZip, cb) {
  ...
  function done () {
    downloadJars.download(function () {
      log.important('Magnolia has been successfully setup for light development!')
      log.important("You can now open a new terminal tab or window and start it up with the CLI command 'mgnl start'")
      log.important('Magnolia will be ready after a few seconds at localhost:8080/magnoliaAuthor. Username and password is superuser')
...
var validateAndResolveArgs = function (program, credentials) {
  ...
  if (typeof program.path === 'undefined') {
    ...
    log.info("No path option provided. Will use the default '%s' in the current directory", program.path)
  }
  ...
  if (program.magnoliaVersion) {
    ...
    log.info('No magnolia-version option provided. Will use the latest stable version.')
  }
  /*
  ...
  if (!fs.existsSync(lightModulesRoot)) {
    log.info("'%s' does not seem to exist. Path will be created automatically.", lightModulesRoot)
